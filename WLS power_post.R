...
WLSpower.post = function(x, moderators, tested.beta){
  ...
  res.mod.fe=rma(es,vi,data=rawdata,level=95,method="FE",mods=as.formula(paste("~",paste(moderators,collapse="+"))))
  ...
  ## Actual beta values
  actual.b.2tail.fe=res.mod.fe$b[1+b,1]
  actual.b.2tail.me=res.mod$b[1+b,1]
  #################################################################################
  ...
  actual.b.2tail.fe=round(actual.b.2tail.fe,digits=2)
  ...
  actual.b.2tail.me=round(actual.b.2tail.me,digits=2)
  #################################################################################
  
  ################## Organizing all results #############################
  WLSpower.table=data.frame(matrix(nrow = 7,ncol = 2))
